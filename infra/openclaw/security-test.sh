...
echo "\n[5] Slack Allowed Test (should WORK if squid allows it)"
curl -I https://slack.com --max-time 8 2>/dev/null && echo "✅ Slack reachable" || echo "❌ Slack blocked"

echo "\n[6] Raw TCP Test (should FAIL)"
nc -zv google.com 443 2>/dev/null && echo "❌ Raw TCP works" || echo "✅ Raw TCP blocked"

echo "\n[7] DNS Test (should FAIL for arbitrary domains if only proxy is allowed)"
